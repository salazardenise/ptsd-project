""" This script seeds the database for the PTSD Project app. """
...
# salt size for salting passwords
SALT_SIZE = 16
...
def load_dummy_data():
    ...
    family = Message(message_type='Family',
                     message='I am not feeling well. I will contact you when I start feeling better.')
    boss = Message(message_type='Co-worker',
                   message='I am out sick today. Please message me if there are any immediate deliverables for today.')
    friend = Message(message_type='Friend',
                     message='I really wanted to see you but I cannot visit with you today. Will get in touch when I am feeling better.')
...
def load_programs_from_va_into_file():
    """ Loads data from VA Web Service into programs.txt file. """

    va_license = os.environ['VA_LICENSE']

    client = Client('https://www.va.gov/webservices/PTSD/ptsd.cfc?wsdl')

    output = client.service.PTSD_Program_Locator_query(va_license)
...
def load_messages():
    """ Load messages info and add to database. """
...
def load_dummy_users_with_rest_of_data(needs_updates):
    """ Loads dummy users with real data for programs, recordings, and messages. """
